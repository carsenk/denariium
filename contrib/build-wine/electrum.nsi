...
;Variables
  ...
  !define PRODUCT_WEB_SITE "https://github.com/spesmilo/denariium"
  !define PRODUCT_PUBLISHER "Denariium Technologies GmbH"
...
;Interface Settings
  ...
  !define MUI_ICON "tmp\denariium\icons\denariium.ico"
...
Section
  ...
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\denariium-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\denariium-${PRODUCT_VERSION}.exe" 0


  ;Links denarius: URI's to Denariium
  WriteRegStr HKCU "Software\Classes\denarius" "" "URL:denarius Protocol"
  WriteRegStr HKCU "Software\Classes\denarius" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\denarius" "DefaultIcon" "$\"$INSTDIR\denariium.ico, 0$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\denariium.ico"
